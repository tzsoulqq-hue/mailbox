...
def main() -> int:
    parser = argparse.ArgumentParser(description="Open a visible Camoufox window for mailbox manual recovery")
    ...
    parser.add_argument("--complete-url", default="")
    ...
    hold_seconds = max(60, int(args.hold_seconds or 7200))
    ...
    with Camoufox(
        ...
        start_url = args.authorize_url or args.url
        ...
        while time.time() < deadline and ctx.pages:
            if args.authorize_url:
                ...
                if code:
                    if not all([args.email, args.client_id, args.redirect_uri, args.scope, args.complete_url]):
                        raise RuntimeError("OAuth completion arguments are incomplete")
