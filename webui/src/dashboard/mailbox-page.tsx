  ...
  return (
    <>
      ...
      <section className="workspace singlePaneWorkspace">
        <div className="panel">
          <MailboxPanel mailboxes={data.mailboxes} domains={data.domains} providerCapabilities={data.providerCapabilities} selected={selectedEmail} busy={data.busy} showSecrets={showSecrets} registering={actions.registering} oauthing={actions.oauthing} manualRecoverying={actions.manualRecoverying} inboxLoading={actions.inboxLoading} domainSyncing={actions.domainSyncing} runningWorkflowByEmail={data.runningByEmail} onSelect={(mailbox) => setSelectedEmail(mailbox.email_address)} onOpenWorkflow={() => actions.toast.showOK('请在工作流页查看任务详情')} onRegisterMailbox={actions.registerMailbox} onOAuth={actions.runOAuth} onManualRecovery={actions.startManualRecovery} onFetchInbox={() => actions.fetchInbox()} onSyncDomains={actions.syncCloudflareDomains} onToggleSecrets={() => setShowSecrets((value) => !value)} onDelete={actions.deleteMailbox} onDone={actions.done} onError={actions.toast.showError} />
        </div>
      </section>
